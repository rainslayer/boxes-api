...
async function adminRoleMiddleware(req, res, next) {
  try {
    ...
    const { success, data } = await AuthService.getAuthenticatedUser(accessToken, refreshToken);
    ...
    return res.status(401).json({ message: 'User must be logged in as admin to perform this action' });
  } catch (e) {
